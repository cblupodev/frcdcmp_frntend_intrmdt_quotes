...
// parse the quotes array on the '@' symbol
quotes = quotes.map(function(elem, idx) {
    return elem.split('@');
});
var quotesWrite = quotes;
...
myApp.controller('MainController',['$scope', function($scope){
    ...
    $scope.pickQuote = function() {
        // pick random index
        var index = Math.floor(Math.random() * quotesWrite.length);
        if (index % 2 !== 0) {
           index--;
        }

        var ret = quotesWrite.splice(index, 1); // catch the quotes that was removed from the array
        $scope.determinateValue = 100 * (numQuotes - quotesWrite.length) / numQuotes; // calc what percentage of quotes they've seen
        if (ret[0] != null) {
            $scope.quote = ret[0][0];
            $scope.cite = ret[0][1];
        } else {
            // all quotes have been seen
            $scope.quote = 'all quotes were displayed. refresh the page to see them again';
            $scope.cite = '';
        }
    };

    // pick a quote when the page loads
    $scope.pickQuote();
}]);
